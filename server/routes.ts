...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // New route to get all transactions for the logged-in user
  app.get('/api/transactions', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      // Use the new storage method to get all transactions for the user
      const transactions = await storage.getAllTransactionsByUserId(userId);
      res.json(transactions);
    } catch (error) {
      console.error("Error fetching all transactions for user:", error);
      res.status(500).json({ message: "Failed to fetch transactions" });
    }
  });
